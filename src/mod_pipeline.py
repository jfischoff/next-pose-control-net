...
from diffusers import FlaxAutoencoderKL, FlaxControlNetModel, FlaxUNet2DConditionModel
...
class FlaxStableDiffusionMultiControlNetPipeline(FlaxDiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        controlnet: List[FlaxControlNetModel],
        scheduler: Union[
    ...
    def prepare_image_inputs(self, image: Union[List[Image.Image], List[List[Image.Image]]]):

        if isinstance(image[0], Image.Image):
            image = [[im] for im in image]

        processed_images = [jnp.stack([preprocess(img, jnp.float32) for img in l]) for l in images]
    ...
    def _generate(
        ...
        base_image = jnp.concatenate([image[0]] * 2)
        image = jnp.concatenate([image[1]] * 2)
